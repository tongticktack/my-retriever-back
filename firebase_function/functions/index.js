...
exports.collectPoliceData = functions
  ...
  .https.onRequest(async (req, res) => {
    try {
        ...
        try {
          ...
          const response = await axios.get(apiUrl, { 
              params: { serviceKey: SERVICE_KEY, 
                ADDR: "경기도", // 지역구
                numOfRows: "30", //데일리로 전날 n개 데이터 db화
                _type: "json" }, 
      ...
      const collectionRef = db.collection("testPortalLostItem3"); //PoliceLostItem 컬렉션에 저장
  ...
  .https.onRequest(async (req, res) => {
    try {
      ...
      const db = admin.firestore();
      const batch = db.batch();
      const collectionRef = db.collection("testPortalLostItem3"); //testPortalLostItem2 컬렉션에 저장
...
exports.addlocations = functions
  .region("asia-northeast3")
    .firestore.document("testPortalLostItem3/{docId}")
