...
exports.collectPoliceData = functions
  ...
  .https.onRequest(async (req, res) => {
    try {
        ...
        try {
          ...
          const response = await axios.get(apiUrl, { 
              params: { serviceKey: SERVICE_KEY, 
                ADDR: "경기도", // 지역구
                numOfRows: "1500", //데일리로 전날 n개 데이터 db화
                _type: "json" }, 
      ...
      const finalItemList = Array.from(yesterdaysItems.values());
      
      const db = admin.firestore();
      const batch = db.batch();
      const collectionRef = db.collection("testPortalLostItem4"); //PoliceLostItem 컬렉션에 저장
  ...
  //     const db = admin.firestore();
  //     const batch = db.batch();
  //     const collectionRef = db.collection("testPortalLostItem4"); //testPortalLostItem2 컬렉션에 저장
...
exports.addlocations = functions
  .region("asia-northeast3")
    .firestore.document("testPortalLostItem4/{docId}")
